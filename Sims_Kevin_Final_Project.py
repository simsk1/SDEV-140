...
def prot():
    ...
    top1.geometry("300x200") #setting the size of the new window which makes it easier to move around and read the complete window title
    ...
    close_prot = Button(top1, text="Close window", command=top1.destroy, padx=10, pady=10).pack(anchor=S) #creating the "Close window" button that when clicked will return the user to the root window.
...
def fat():
    ...
    close_fat = Button(top2, text="Close window", command=top2.destroy, padx=10, pady=10).pack(anchor=S)

def cho():
    c = IntVar()
    top3 = Toplevel()
    top3.geometry("300x200")
    ...
    close_cho = Button(top3, text="Close Window", command=top3.destroy, padx=10, pady=10).pack(anchor=S)
